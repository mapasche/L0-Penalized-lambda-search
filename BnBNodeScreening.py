...
def dual_initial_problem (y, A, S0, S1, S, alpha = 1, M = 1000000):
    
    n = y.shape[0]
    u = cp.Variable((n, 1))
    obj =  0.5 * cp.sum_squares(u)
    obj += - 0.5 * cp.sum_squares(y - u) 
    ...
    #eliminate 0
    u_vector = u.value
    u_vector = np.where(abs(u_vector) < 1e-7, 0, u_vector )
    return result, u_vector
